...
def detect_b2(klines: list[dict], index: int, kirin_context: Optional[dict] = None) -> Optional[StrategySignal]:
    ...
    if today.get("boll_upper") and today.get("boll_lower"):
        ...
        prev_width = (
            (yesterday["boll_upper"] - yesterday["boll_lower"]) / yesterday["boll_mid"] if yesterday["boll_mid"] else 0
        )
    ...
    if today.get("dmi_plus") and yesterday.get("dmi_minus"):
        if _safe_num(yesterday.get("dmi_plus")) < _safe_num(yesterday.get("dmi_minus")) and _safe_num(
            today.get("dmi_plus")
        ) > _safe_num(today.get("dmi_minus")):
            confidence += 0.10
