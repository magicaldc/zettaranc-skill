...
class TrackingSyncer:
    ...
    def sync_daily(self, ts_code: str, days: int = 365) -> Dict[str, Any]:
        ...
        try:
            ...
            with get_connection() as conn:
                ...
                # 转换为 DataFrame 格式
                import pandas as pd
                kline_data = pd.DataFrame(kline_records, columns=['trade_date', 'open', 'high', 'low', 'close', 'vol', 'pct_chg', 'amount'])
            ...
            with get_connection() as conn:
                ...
                for i, row in enumerate(kline_records):
                    ...
                    """, (
                        ...
                        row.get('vol'), row.get('pct_chg'), row.get('amount'),
            ...
            return {
                "success": True,
                "message": f"已同步 {ts_code} 的 {saved_count} 条记录",
    ...
    def _detect_signal(self, indicator_data: Dict[str, Any], kline_data: Dict[str, Any] = None, 
                       prev_kline_data: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        检测交易信号（根据 Z 哥的策略）
